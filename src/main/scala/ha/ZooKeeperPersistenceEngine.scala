...
private  class ZooKeeperPersistenceEngine(val serializer: Serializer)
  extends PersistenceEngine {
  private val WORKING_DIR = "/spark/status"  //存储状态数据
  private val zk: CuratorFramework = SparkCuratorUtil.newClient()
  ...
  private def deserializeFromFile[T](filename: String)(implicit m: ClassTag[T]): Option[T] = {
    ...
    try {
      ...
      case e: Exception =>
        println("Exception while reading persisted file, deleting", e)
